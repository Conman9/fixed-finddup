...
#define OPTSTR	"lh"
#endif
#define SORT qsort((filedesc *)filelist, n_files, sizeof(filedesc), comp1);
...
static double HelpLen = sizeof(HelpMsg)/sizeof(char *);
...
int comp1(char *p1, char *p2;);					/* compare two filedesc's */
...
struct option longopts[] = {
  ...
  // { "verbose", optional_argument,      NULL, 1   },
};
...
char *argv[];
{
	...
	while((ch = getopt_long(argc, argv, "ldh", longopts, NULL)) != EOF){
		switch (ch) {
...

	...
	while (loc = ftell(namefd), getline(&curfile, &bufsize, namefd) != -1) { //fgets(curfile, MAXFN, namefd) != NULL) {
		...
		int non_exist = stat(curfile, &statbuf);
		if (non_exist || !S_ISREG(statbuf.st_mode)) {
...
char *p1, *p2;
{
	...
	if(p1a->length != p2a->length) return (p1a->length - p2a->length);
	else if(p1a->crc32 != p2a->crc32) return (p1a->crc32 - p2a->crc32);
	else if(p1a->device != p2a->device) return (p1a->device - p2a->device);
	else if(p1a->inode != p2a->inode) return (p1a->inode - p2a->inode);

	//if((retval = p1a->length - p2a->length) ||
...
scan2() {
	int ix, ix2, lastix;
	int inmatch = 0;				/* 1st filename has been printed */
...
{
	register filedesc *p1, *p2;
	int ix, inmatch=0, need_hdr = 1, ix2;
...
int ix;
{
	...
	val1 = rc_crc32(0, buffer, strlen(buffer));
	...
	//while ((ch = fgetc(fp)) != EOF) {
		...
		//val1 += (val1 << 1) ^ (ch + carry);  /////CHANGED VALUE TO (ch + carry)
		//val2 += ch << (ch & 003);
	//}
	debug(("v1: %08lx v2: %08lx ", val1, val2));

	return val1;//((val1 & 0xffff) << 12) ^ (val2 && 0xffffff);
}

